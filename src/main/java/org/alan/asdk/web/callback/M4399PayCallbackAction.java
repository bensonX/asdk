...
public class M4399PayCallbackAction extends UActionSupport {
    ...
    public void payCallback() {
        try {
            Log.i("<4399>充值回调开始： orderid=" +orderid+ ", p_type=" +p_type+ ", uid=" +uid+ ", money=" +money+ ", gamemoney=" +gamemoney+ ", serverid=" +serverid+ ", mark=" +mark+ ", roleid=" +roleid+ ", time=" +time+ ", sign=" +sign);
            ...
            if (order == null || order.getChannel() == null) {
                Log.d("<4399>订单["+orderID+"]不存在 , 或者 cannel 不存在");
                this.renderState(1, "other_error", "cpOrderId 错误");
                return;
            }

            if (order.getState() == PayState.STATE_COMPLETE) {
                Log.d("<4399>订单["+orderID+"] , 订单重复 getState = "+ order.getState());
                this.renderState(3, null, "该订单已经被处理,或者CP订单号重复");
                return;
            }
            int _money = new Double(Double.parseDouble(money)*100).intValue();
            if (_money!=order.getMoney()){
                Log.d("<4399>订单["+orderID+"] , 订单金额(order.getMoney="+order.getMoney()+")与传入金额["+money+"]不一致");
                return;
            }
    ...
    private boolean isValid(UChannel channel) throws Exception {
        ...
        Log.i("<4399>充值验证 :" + sb.toString()+", The sign is " + sign + "; the vCode is " + vCode);
