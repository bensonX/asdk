...
public class MeiZuPayCallbackAction extends UActionSupport {
    ...
    public void payCallback() {
        try {
            Log.i("<魅族>充值回调开始： notify_time=" +notify_time+ ", notify_id=" +notify_id+ ", order_id=" +order_id+ ", app_id=" +app_id+ ", uid=" +uid+ ", partner_id=" +partner_id+ ", cp_order_id=" +cp_order_id+ ", product_id=" +product_id+ ", product_unit=" +product_unit+ ", buy_amount=" +buy_amount+ ", product_per_price=" +product_per_price+ ", total_price=" +total_price+ ", trade_status=" +trade_status+ ", create_time=" +create_time+ ", pay_time=" +pay_time+ ", pay_type=" +pay_type+ ", user_info=" +user_info+ ", sign=" +sign+ ", sign_type=" +sign_type);
            ...
            if (order.getState() == PayState.STATE_COMPLETE) {
                Log.d("<魅族>订单完成状态：" + order.getState());
                this.renderState(true);
                return;
            }

            if (!isSignOK(channel)) {
                Log.d("<魅族>签名错误.sign:%s;appKey:%s;orderID:%s", sign, channel.getCpPayKey(), cp_order_id);
                this.renderState(true);
                return;
            }
