...
<div class="container">
    <div class="row mg-t">
        ...
        <div class="col-md-6">
            <form id="form" class="form-horizontal" action="<%=basePath%>/account/bindEmail"
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="email"
                           ...
                           data-bv-notempty-message="${register_text_5}">
                    </div>
                </div>

                <input type="hidden" name="oldPsw">
                <!--密码-->
                <div class="form-group">
                    <div class="col-xs-12">
                    <input type="password" class="form-control" name="psw"
                           placeholder="${Login_text_3}"
                           data-bv-notempty="true"
                ...
                <!--绑定-->
                <button type="submit" class="btn btn-info btn-block">${identify_text_23}</button>
                <!--返回-->
                ...
                $("#form").bootstrapValidator({
                    ...
                    submitHandler: function (validator, form) {
                        ...
                        $.post(form.attr('action'), form.serialize(), function (result) {
                            submit.html("${identify_text_23}").attr('disabled', false);//注册
                            if (result.state != 1) {
                                ...
                                if(result.state==5||result.state==6){//获取账户信息失败|该账户已经绑定了邮箱
                                    validator.updateElementStatus($("[name=username]"),"INVALID",msg)
                                }
                                if(result.state==10){//该邮箱已经被使用
                                    validator.updateElementStatus($("[name=email]"),"INVALID",msg)
                                }
                                return;
                            }
                            bootbox.alert("${action_bind_6}", function () {//邮箱绑定成功
                                window.location = "<%=basePath%>/login.jsp?username=" + $("[name=email]").val() + "&psw=" + $("[name=psw]").val();
                            });
                        }, 'json');
...
//            return "{\"channel\":\"1\",\"userName\":\"p1XgkGB8\",\"appID\":\"1\",\"pwd\":\"e9044a0f8fada3d5e434a264fe34fc11\"}";
//        },
//        onLoginFromJavaScript: function () {
//            alert("登录成功");
//        }
//    };
    var onReady = function () {
        ...
        if(!userInfo.userName){
            bootbox.alert("${identify_text_24}", function () {//只有游客玩家才能绑定邮箱
                window.location = "<%=basePath%>/login.jsp";
            });
            return;
        }
        $("[name=username]").val(userInfo.userName);
        $("[name=oldPsw]").val(userInfo.pwd);
    };
