...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">自渠道玩家管理</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <form id="toolbar" class="form-inline" action="<%=basePath%>/admin/users/getAllUsers">
                        <div class="form-group">
                            <input class="form-control" size="10" type="tel" name="currUserID" placeholder="Open ID">
                        </div>
                        <button type="button" class="btn btn-default" id="subBtn">查询</button>
                    </form>
                    <table id="tables" data-toggle="table"
                           data-url="<%=basePath%>/admin/users/getAllUsers"
                           data-page-size="100"
                        ...
                        <tr>
                            ...
                            <th data-field="id">Open ID</th>
                            <th data-field="name">名称</th>
                            <th data-field="appName">游戏</th>
                            <th data-field="channelName">所属渠道</th>
                            <th data-field="channelUserID">渠道 User ID</th>
                            <th data-field="channelUserName">渠道用户名</th>
                            <th data-field="channelUserNick">用户昵称</th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#subBtn').click(function(){
                //检查是否为数字
                var v = $("[name=currUserID]").val();
                if (!$.isNumeric(v)){
                    bootbox.alert('错误!<br>Open ID 必须为纯数字');
                    return;
                }
                if(v > 2147483648){
                    bootbox.alert('错误!<br>Open ID 数字过大');
                    return;
                }
                $('#toolbar').ajaxSubmit({
        ...
        window.actionEvents = {
            'click .remove': function (e, value, row) {
                var id = row.id;
                bootbox.confirm("你确定要删除[玩家:"+row.name+"]吗 , 此操作不可恢复", function(result){
                    if(result==true){
                        $.ajax({
                            url:'<%=basePath%>/admin/users/removeUser',
                            data:{
                                currUserID:id
                            },
                            type:"POST",
